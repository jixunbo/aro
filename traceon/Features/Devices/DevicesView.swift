    ...
    private var setupCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if connectivity.isWatchAppInstalled {
                Text("手表组件已安装，正在等待第一条电量数据。通常不需要额外设置；如果几分钟后仍无数据，请在手表上打开一次 Companio。")
            } else {
                Text("Companio 已包含 Apple Watch 组件。若系统没有自动安装，请在 iPhone 的 Watch App → 我的手表 → 可用 App 中安装 Companio；开启自动安装后无需手动配对。")
            }
        ...
        VStack(spacing: 18) {
            ...
            VStack(spacing: 5) {
                ...
                if let date = store.snapshot?.updatedAt {
                    ...
                    Text("请在 Apple Watch 上打开一次 Companio")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
            }
