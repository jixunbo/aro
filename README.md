...
2. 在 `traceon` 和 `Companio Watch App` 两个 target 的 Signing & Capabilities 中选择同一个开发团队。
3. 保持 iOS bundle identifier 为 `com.xunbo.traceon`，watchOS bundle identifier 为 `com.xunbo.traceon.watchkitapp`，并确认 watch target 的 `WKCompanionAppBundleIdentifier` 为 `com.xunbo.traceon`。
4. 选择已配对 Apple Watch 的 iPhone 作为运行设备，并运行共享的 `traceon` scheme。watchOS App 会作为 Watch Content 嵌入 iOS App。
5. 首次启动依次授予“使用 App 时”和“始终”定位权限。若设备页迟迟没有首条电量，请在 Apple Watch 上打开一次 Companio。
