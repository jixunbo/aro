...
6. 选择已配对 Apple Watch 的 iPhone 作为运行设备，并运行共享的 `aro` scheme。watchOS App 会作为 Watch Content 嵌入 iOS App，Widget Extension 会嵌入 Watch App。首次启动依次授予“使用 App 时”和“始终”定位权限；若设备页迟迟没有首条电量，请在 Apple Watch 上打开一次 aro，随后在表盘编辑器中选择 `aro 电量`。
