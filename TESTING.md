...
1. **普通后台**：打开 traceon 后切换到其他 App，步行 500 米；回到 traceon，确认轨迹点、距离和时间更新。
2. **锁屏**：锁屏后步行或驾车 10 分钟；解锁后确认路线没有完全中断。
3. **系统回收**：开启多个大型 App 造成内存压力，随后移动；traceon 应能通过显著位置变化或到访通道恢复。
4. **手动划掉**：从多任务界面结束 traceon，再移动至少 500 米；不同 iOS 版本的调度可能有延迟，检查“记录通道”是否显示“显著位置变化”。
...
- 第一天：暂停 traceon，记录“设置 → 电池”中的总耗电与屏幕时间。
- 第二天：使用“均衡”模式，记录相同数据以及 traceon 的后台活动占比。
