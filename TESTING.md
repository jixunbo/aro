...
4. **均衡同路线对照**：在相近时间和信号环境下，用 aro“均衡”和另一款足迹 App 的省电模式同时记录同一条约 2–3 km 路线。比较总距离、拐弯保留、轨迹点精度和点源；aro 新增点应来自标准定位，不能再把 `significant`/`visit` 的粗糙坐标直接连进路线，且均衡模式不应接受水平精度差于约 100 m 的新点。
5. **极省电事件唤醒**：使用“极省电”模式静止一段时间后移动，等待系统显著位置变化或到访事件触发。确认 aro 平时没有持续标准定位，事件只负责唤醒短暂定位窗口；最终写入的新增路线点应显示为标准定位，水平精度不应差于约 150 m。若一次唤醒没有拿到足够好的定位，允许这次不新增点，但不能把粗糙的 wake/visit 坐标直接写入轨迹形成长直线或三角形。
6. **系统回收**：开启多个大型 App 造成内存压力，随后移动；aro 应能通过显著位置变化或到访通道恢复。
7. **手动划掉**：从多任务界面结束 aro，再移动至少 500 米；不同 iOS 版本的调度可能有延迟，确认重新获得系统定位事件后仍能继续写入标准定位点。不要要求手动划掉后的固定唤醒时延。
8. **重启手机**：重启并首次解锁后移动；确认数据库仍可写入。
9. **权限降级**：分别测试“使用 App 时”、关闭精确位置、关闭系统定位和关闭后台 App 刷新，确认界面能明确显示状态。
10. **弱信号**：测试地铁、地下停车场、高楼密集区；确认漂移点不会造成异常长距离，极省电模式在定位质量不足时宁可跳过一次采样。
...
- 第三天：使用“极省电”模式，确认标准定位主要只在系统 wake 后短暂出现，并记录相同电量数据；如需路线细节，再单独测试“精确”。
- 对“均衡”模式额外做一次静止 30 分钟 → 移动 30–60 分钟的 Energy Log，确认标准定位自动暂停后 Core Motion 活动更新也停止，并在低功耗移动事件出现后恢复标准定位。
- 对“极省电”模式做一次 1–2 小时的 Energy Log，确认显著位置变化/到访监听之间没有持续 standard-location 活动；每次 wake 后的标准定位 burst 应短暂结束，而不是一直运行。
- 使用 Xcode Instruments 的 Energy Log 做 30–60 分钟步行和驾车样本。
- 测试低电量模式、Wi‑Fi 开/关和蜂窝弱信号；无线信号差通常会放大定位耗电。

建议初始目标：均衡模式优先保证单点质量，同时保持明显低于运动模式的采样密度；极省电模式优先保证全天低功耗，允许系统调度导致更稀疏的路线。不要在产品文案中承诺固定耗电百分比或固定采样间隔。
...
2. 在手表上打开 aro，确认电量、充电状态、iPhone 连接状态和底部的 bundle-derived 版本号出现；`1.4.4` 的初始构建应显示 `v1.4.4 (1)`。回到 iPhone“设备”页，确认设备名、电量和同步时间一致。
